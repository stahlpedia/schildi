...
export default function Skills({ projectId, onNavigateToKanban }) {
  ...
  // Editor tabs
  const [editorTab, setEditorTab] = useState('html') // html | css | fields | use
  ...
  const loadTemplates = async () => {
    try {
      const list = await templates.list(projectId)
  ...
  const selectTemplate = (tpl) => {
    ...
    setForm({
      ...
      preview_data: tpl.preview_data || {}
    })
    // Set field values from preview_data
    const vals = {}
  ...
  const addField = () => {
    setForm(f => ({ ...f, fields: [...f.fields, { name: '', type: 'text', label: '' }] }))
  }
  ...
  return (
    <div className="w-full max-w-full overflow-hidden flex flex-col" style={{ height: 'calc(100vh - 120px)' }}>
      ...
      <CardModal
        ...
        defaultTitle={selectedTemplate ? `Template: ${selectedTemplate.name}` : ''}
