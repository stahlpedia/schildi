...
export default function ContentBrowser() {
  ...
  const handleDelete = async (entry) => {
    const typ = entry.isDirectory ? 'Ordner' : 'Datei'
    if (!confirm(`${typ} "${entry.name}" löschen?`)) return
    try {
      await apiFetch('/delete?path=' + encodeURIComponent(entry.path), { method: 'DELETE' })
      if (selectedEntry?.path === entry.path) { setSelectedEntry(null); setPreviewContent(null) }
      loadDir(currentPath)
    } catch (e) { alert('Fehler: ' + e.message) }
  }
  ...
  const breadcrumbs = [{ label: 'Content', path: '' }]
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 bg-gray-900 rounded-xl border border-gray-800 px-4 py-3 mb-4 shrink-0">
            ...
            <React.Fragment key={i}>
              ...
              <button
                ...
                className={`text-sm whitespace-nowrap transition-colors ${
                  i === breadcrumbs.length - 1 ? 'text-white font-medium' : 'text-gray-400 hover:text-white'
                }`}
              >
                {bc.label}
              </button>
        ...
        <div className="flex gap-2 shrink-0">
          <button onClick={() => setShowNewFolder(true)}
            className="px-3 py-2 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-lg text-xs font-medium transition-colors">+ Ordner</button>
          <input type="file" ref={fileInputRef} onChange={handleUpload} className="hidden" multiple />
          <button onClick={() => fileInputRef.current?.click()} disabled={uploading}
            className="px-3 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 rounded-lg text-xs font-medium transition-colors">
      ...
      {showNewFolder && (
        <div className="flex gap-2 mb-3 px-1">
          <input value={newFolderName} onChange={e => setNewFolderName(e.target.value)} placeholder="Ordner-Name"
            className="flex-1 max-w-xs px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm text-white focus:outline-none focus:border-emerald-500"
            onKeyDown={e => { if (e.key === 'Enter') handleCreateFolder(); if (e.key === 'Escape') setShowNewFolder(false) }} autoFocus />
          <button onClick={handleCreateFolder} className="px-3 py-2 bg-emerald-600 hover:bg-emerald-500 rounded-lg text-xs font-medium">Erstellen</button>
          <button onClick={() => setShowNewFolder(false)} className="px-3 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-xs">Abbrechen</button>
        </div>
      )}
        ...
        <div className="flex-1 bg-gray-900 rounded-xl border border-gray-800 flex flex-col overflow-hidden">
          {loading ? (
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {currentPath && (
                <button onClick={goUp}
                  className="w-full flex items-center gap-3 px-4 py-3 text-left border-b border-gray-800/50 hover:bg-gray-800/50 transition-colors text-gray-400">
              ...
              {entries.length === 0 && !currentPath && (
                <div className="flex flex-col items-center justify-center py-20 text-gray-500">
                  ...
                  <div className="text-lg">Content-Verzeichnis ist leer</div>
                  <p className="text-sm mt-2">Lade Dateien hoch oder erstelle Ordner</p>
                </div>
              )}
              ...
              {entries.map(entry => (
                <div key={entry.path}
                  ...
                  className={`flex items-center gap-3 px-4 py-3 border-b border-gray-800/50 cursor-pointer hover:bg-gray-800/50 transition-colors group ${
                    selectedEntry?.path === entry.path ? 'bg-gray-800' : ''
                  }`}>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); handleDelete(entry) }}
                    className="opacity-0 group-hover:opacity-100 p-1 text-gray-500 hover:text-red-400 transition-all shrink-0" title="Löschen">
                    🗑️
                  </button>
                </div>
              ))}
        ...
        {selectedEntry && !selectedEntry.isDirectory && (
          <div className="hidden md:flex w-96 bg-gray-900 rounded-xl border border-gray-800 flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto flex items-center justify-center bg-gray-800/50">
              {isImage(selectedEntry.name) ? (
                ...
                <div className="p-8 text-center">
                  ...
                  <audio src={fileUrl(selectedEntry.path)} controls className="w-full" />
                </div>
              ) : previewContent !== null ? (
                ...
                <div className="text-center text-gray-500 p-8">
                  <div className="text-5xl mb-4">{getIcon(selectedEntry)}</div>
                  <div className="text-sm">{selectedEntry.name}</div>
                </div>
              )}
            </div>
            <div className="p-3 border-t border-gray-800 flex items-center justify-between text-xs text-gray-500">
              ...
              <a href={fileUrl(selectedEntry.path)} download={selectedEntry.name}
                className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 rounded text-xs text-white transition-colors">Download</a>
            </div>
          </div>
        )}
      </div>
