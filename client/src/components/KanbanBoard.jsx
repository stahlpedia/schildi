...
export default function KanbanBoard(props = {}) {
  ...
  const moveCard = async (card, direction) => {
    ...
    if (direction === 'left' && currentColIndex > 0) {
      await kanban.update(card.id, { column_name: cols[currentColIndex - 1].name })
      load()
  ...
  return (
    <div>
      ...
      {/* Board - Desktop: Grid, Mobile: Horizontal Scroll */}
            ...
            <div key={col.id} className={`bg-gray-900 rounded-xl border-t-4 ${col.color} p-4 min-h-[400px] flex-shrink-0 w-80`}
              ...
              <div className="space-y-3">
                {colCards(col.name).map((card, index) => {
                  ...
                  return (
                    <div key={card.id}
                      className={`bg-gray-800 p-3 rounded-lg border transition-colors ${
                        card.on_hold ? 'border-yellow-500/50 bg-gray-800/50' : 'border-gray-700'
                      }`}>
                      ...
                      <div className="flex gap-1 mb-2">
                        {canMoveLeft && (
                          <button onClick={() => moveCard(card, 'left')}
                            ...
                            title={`Nach ${cols[currentColIndex - 1].label}`}>
                      ...
                      <div className="flex gap-1 mb-2">
                        <button onClick={() => handleExecute(card)} disabled={executing === card.id || card.on_hold}
                          className="px-2 py-1 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed rounded text-xs font-medium transition-colors"
                          title="🐢 Schildi ausführen">
